...
import { DeepPartial } from '../../src/Models/Utility';
import traversalFlatten from '../../src/Helpers/flattenTraversalNodeCollection';
...
export default function createTraversalModel(
  partialResponse: DeepPartial<TraversalsResponse> = {}
): TraversalModel {
  const response = createResponse(partialResponse);
  return traversalFlatten(response);
}
