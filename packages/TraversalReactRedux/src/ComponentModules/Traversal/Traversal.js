...
export const TraversalResponse =  ({ question, answers, error, actions = defaultTraversalActions, components = defaultTraversalComponents }) => {
...
export const TraversalTable = ({ node, questions, answers, errors, actions = defaultTraversalActions, components = defaultTraversalComponents }) => {
    const comps = { ...defaultTraversalComponents, ...components };
...
export const TraversalForm = ({ traversal, containerRef, children, minWidthTable = 700, actions = defaultTraversalActions, components = defaultTraversalComponents }) => {
    const comps = { ...defaultTraversalComponents, ...components };
    const { algoId, nodeIds, nodes, questions, answers, errors, previous, loading, collectionErrors, minHeight } = traversal;
    return (<comps.Form onSubmit={(e) => { e.preventDefault(); actions.next(); }} id="Traversal">
        <comps.Container ref={containerRef} style={{minHeight: minHeight + "px"}}>
            ...
            <comps.Collection onRest={actions.setHeight}>
                {nodes && nodeIds.map((nodeId) => {
                    ...
                    return (<comps.Fieldset key={`${algoId}_${nodeId}`} mirror={previous} disabled={loading} initialPose="preEnterpose">
...
export const Traversal = ({ traversal, containerRef, minWidthTable = 700, labels = defaultLabels.traversal, actions = defaultTraversalActions, components = defaultTraversalComponents }) => {
    const comps = { ...defaultTraversalComponents, ...components };
    return (<Container>
        <TraversalForm containerRef={containerRef} traversal={traversal} minWidthTable={minWidthTable} actions={actions} components={comps}>
            <Buttons>
                <ButtonGroup>
                    <PrevButton>
                        <comps.Button 
